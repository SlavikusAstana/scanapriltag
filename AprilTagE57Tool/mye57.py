...
SUPPORTED_SPHERICAL_POINT_FIELDS = {
    "sphericalRange": np.float64,
    "sphericalAzimuth": np.float64,
    "sphericalElevation": np.float64,
}
    ...
    def read_scan(self,
        ...
        if transform:
            if coordinate_system is COORDINATE_SYSTEMS.CARTESIAN:
                ...
                data["sphericalRangeIs0"] = np.zeros(data["sphericalRange"].size, dtype=np.bool_)
                data["sphericalRangeIs0"][data["sphericalRange"] == 0] = 1
                data["sphericalRange"][data["sphericalRange"] == 0] = 1000
            ...
            if header.has_pose():
                try:
                    header_rotation = header.rotation
                except:
                    header_rotation = np.array([1.0, 0.0, 0.0, 0.0])
                try:
                    header_translation = header.translation
                except:
                    header_translation = np.array([0.0, 0.0, 0.0])
                xyz = self.to_global(xyz, header_rotation, header_translation)
